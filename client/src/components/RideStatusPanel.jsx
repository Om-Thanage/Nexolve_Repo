...
export default function RideStatusPanel({
  ...
  // Determine Receiver ID for Chat
  const receiverId = isDriver ? request.user?._id : ride.host?.user?._id;
  const receiverName = isDriver ? request.user?.name : ride.host?.user?.name;
    ...
    >
      ...
      <div className="bg-primary text-primary-foreground p-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {/* Chat Button (Only visible in active statuses) */}
